-- =====================================================
-- MIGRACIÓN UNIFICADA: CATEGORIES + MEJORAS CRÍTICAS
-- =====================================================
-- Este script unifica:
-- 1. Creación/actualización de tabla categories con columna 'type'
-- 2. Mejoras críticas: funciones, triggers, alertas automáticas
-- =====================================================
...
-- =====================================================
-- PARTE 1: CREAR/ACTUALIZAR TABLA CATEGORIES
-- =====================================================
...
-- Verificar y agregar columna 'icon' si no existe
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 
...
-- Verificar y agregar columna 'color' si no existe
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 
...
-- =====================================================
-- PARTE 2: FUNCIÓN PARA CREAR ALERTAS (Service Role)
-- =====================================================
...
-- =====================================================
-- PARTE 3: TRIGGER: ALERTA CUANDO PRESUPUESTO SUPERA 80%
-- =====================================================
...
-- Trigger para transacciones (solo si la tabla transactions existe)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
        ...
        RAISE NOTICE '✅ Trigger de presupuesto creado';
    ELSE
        RAISE NOTICE '⚠️ Tabla transactions no existe. Trigger de presupuesto no se creó.';
    END IF;
END $$;

-- =====================================================
-- PARTE 4: TRIGGER: ALERTA DE GASTO INUSUAL
-- =====================================================
...
-- Trigger para gastos inusuales (solo si la tabla transactions existe)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
        ...
        RAISE NOTICE '✅ Trigger de gasto inusual creado';
    ELSE
        RAISE NOTICE '⚠️ Tabla transactions no existe. Trigger de gasto inusual no se creó.';
    END IF;
END $$;

-- =====================================================
-- PARTE 5: TRIGGER: ALERTA DE PROGRESO DE META
-- =====================================================
...
-- Trigger para metas (solo si la tabla goals existe)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'goals') THEN
        ...
        RAISE NOTICE '✅ Trigger de progreso de metas creado';
    ELSE
        RAISE NOTICE '⚠️ Tabla goals no existe. Trigger de progreso de metas no se creó.';
    END IF;
END $$;

-- =====================================================
-- PARTE 6: FUNCIÓN: LIMPIAR ALERTAS EXPIRADAS
-- =====================================================
...
-- =====================================================
-- PARTE 7: FUNCIONES DE AGREGACIÓN PARA DASHBOARDS
-- =====================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'transactions') THEN
        ...
        WHERE type = 'expense';
        RAISE NOTICE '✅ Índice de transacciones creado';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'alerts') THEN
        ...
        RAISE NOTICE '✅ Índice de alertas no leídas creado';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'shared_expense_splits') THEN
        ...
        RAISE NOTICE '✅ Índice de gastos compartidos pendientes creado';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'budgets') THEN
        ...
        RAISE NOTICE '✅ Índice de presupuestos activos creado';
    END IF;
END $$;

-- =====================================================
-- PARTE 9: VALIDACIÓN MEJORADA PARA SPLITS
-- =====================================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'shared_expense_splits') THEN
        ...
        RAISE NOTICE '✅ Trigger validate_expense_splits_trigger eliminado';
    END IF;
END $$;

-- Ahora eliminar y recrear la función
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'shared_expense_splits') THEN
        ...
        RAISE NOTICE '✅ Trigger validate_expense_splits_trigger recreado';
    END IF;
END $$;

-- =====================================================
-- PARTE 10: FUNCIÓN PARA AUTO-CREAR MEMBER AL CREAR GRUPO
-- =====================================================
...
BEGIN
    ...
    ON CONFLICT (family_group_id, user_id) DO NOTHING; -- Evitar duplicados

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger para auto-agregar creador (solo si las tablas existen)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'family_groups')
       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'family_group_members') THEN
        ...
        RAISE NOTICE '✅ Trigger de auto-agregar creador como admin creado';
    ELSE
        RAISE NOTICE '⚠️ Tablas de grupos familiares no existen. Trigger no se creó.';
    END IF;
END $$;

-- =====================================================
-- PARTE 11: FUNCIÓN PARA AUTO-CREAR PREFERENCIAS
-- =====================================================
...
-- =====================================================
-- ✅ MIGRACIÓN UNIFICADA COMPLETADA
-- =====================================================
-- Resumen:
-- ✅ Tabla categories creada/actualizada con columna 'type'
-- ✅ Función create_alert() para crear alertas desde backend
