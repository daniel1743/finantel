...
const Support = () => {
  ...
  // Check if user has premium plan (familiar or enterprise)
  const hasPremiumPlan = ['familiar', 'Familiar', 'family', 'enterprise'].includes(userPlan);
              ...
              <li className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <span className="text-xs">
                    {hasPremiumPlan ? 'De lunes a domingo' : '⭐ Solo planes Familiar y Enterprise'}
                  </span>
                </div>
              </li>
            </ul>
            ...
            <div className="space-y-4">
              {tickets.map((ticket) => (
                <div
                  ...
                  className="border border-gray-100 dark:border-white/5 rounded-2xl p-4 hover:border-[#1C8FA0]/40 hover:shadow-md transition-all cursor-pointer group"
                >
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                    <div className="flex-1">
                      <p className="text-sm text-[#6E6E73] dark:text-gray-400">
                        Ticket #{ticket.id.slice(0, 8)} • {new Date(ticket.created_at).toLocaleDateString('es-ES', { dateStyle: 'medium' })}
                      </p>
                  ...
                  <p className="mt-2 text-sm text-[#6E6E73] dark:text-gray-400 line-clamp-2">{ticket.message}</p>
                  <div className="mt-3 flex flex-wrap gap-4 text-xs text-[#94A3B8] dark:text-gray-500">
                    ...
                    {ticket.sla_hours && <span>SLA: {ticket.sla_hours}h</span>}
                  </div>
                </div>
              ))}
            </div>
