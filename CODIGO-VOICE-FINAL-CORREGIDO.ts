...
// VERSIÓN FINAL CORREGIDA v2 - Parser completo
// ✅ ARREGLA: Bug donde "$28,000" se detectaba como 28
// ✅ ARREGLA: Ahora detecta INGRESOS vs GASTOS automáticamente
...
function parseTranscript(text: string, userCurrency: string = 'CLP') {
  ...
  // Si no se detectó nada, por defecto es gasto
  if (transactionType === 'expense' && !expenseKeywords.some(k => lowerText.includes(k))) {
    console.log('⚠️ Tipo no especificado → Por defecto: GASTO');
  }

  // ==========================================
  // 2. DETECTAR NIVEL DE NECESIDAD
  // ==========================================
  ...
  // ==========================================
  // 3. DETECTAR MONTO (MEJORADO Y CORREGIDO)
  // ==========================================

  // 🔧 NUEVO Patrón 0: Formato monetario "$28,000" o "$28.000" (PRIMERO)
  // Este es el patrón que estaba fallando
  if (!amount) {
    const moneyFormatMatch = text.match(/\$\s*(\d{1,3}(?:[.,]\d{3})+)/i);
    if (moneyFormatMatch) {
      const cleanNumber = moneyFormatMatch[1].replace(/[.,]/g, '');
      amount = parseInt(cleanNumber);
      console.log(`✅ Monto detectado (formato monetario $X,XXX): ${amount} ${userCurrency}`);
    }
  }
  ...
  // Patrón 3: Números con separadores SIN $: "50.000", "50,000"
  if (!amount) {
  ...
  // Patrón 4: Números directos grandes: "5000", "50000" (4+ dígitos)
  if (!amount) {
    const directMatch = lowerText.match(/\b(\d{4,})\b/);
    if (directMatch) {
      amount = parseInt(directMatch[1]);
      console.log(`✅ Monto detectado (directo 4+ dígitos): ${amount} ${userCurrency}`);
    }
  }
  ...
  // Patrón 6: Números pequeños de 1-3 dígitos (ÚLTIMO recurso)
  if (!amount) {
    const smallNumberMatch = lowerText.match(/\b(\d{1,3})\b/);
    if (smallNumberMatch) {
      amount = parseInt(smallNumberMatch[1]);
      console.log(`⚠️ Monto detectado (número pequeño): ${amount} ${userCurrency} - Verifica si es correcto`);
    }
  }

  // ==========================================
  // 4. DETECTAR DESCRIPCIÓN (MEJORADO)
  // ==========================================

  // Remover palabras de monto del texto
  let cleanText = text // Usar texto original (no lowerText) para preservar mayúsculas
    .replace(/\$\s*\d{1,3}(?:[.,]\d{3})+/gi, '') // Remover "$28,000"
    .replace(/\d+\s*mil(?:\s+pesos)?/gi, '')
    .replace(/\d+\s*k\b/gi, '')
    .replace(/\d{1,3}(?:[.,]\d{3})+/g, '')
    .replace(/\b\d{3,}\b/g, '')
    .replace(/hola|gasté|pagué|compré|di|en|de|la|el|los|las|un|una/gi, ' ')
  ...
  if (!description || description.length < 2) {
    const words = lowerText
      .replace(/hola|gasté|pagué|compré|di|pesos|clp|mil|k|necesario|importante|opcional/gi, '')
      .replace(/\d+/g, '')
      .replace(/\$/g, '')
      .trim()
      .split(/\s+/)
  ...
  // ==========================================
  // 5. CLASIFICAR CATEGORÍA Y NECESIDAD
  // ==========================================
  ...
  return {
    ...
    category,
    necessityLevel,
    type: transactionType, // ✅ AHORA USA EL TIPO DETECTADO (income o expense)
  };
}
...
function classifyMerchant(text: string): { category: string; autoNecessity: string } {
  ...
  const categoryMap: { [key: string]: [string, string][] } = {
    ...
    'Alimentación': [
      ...
      ['mercado', 'essential'], ['arepa', 'essential'], ['empanada', 'essential']
    ],
    'Salud': [
  ...
  try {
    ...
    if (!categoryId) {
      ...
      if (categoryId) {
        ...
        console.error('❌ No existe categoría "Otros" para este usuario');
      }
    }
