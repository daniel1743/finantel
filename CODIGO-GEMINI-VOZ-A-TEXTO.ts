...
// Procesa audio → GEMINI (voz a texto) → NLP → Transaction
// ✅ VERSIÓN CON GEMINI API
...
serve(async (req) => {
  ...
  try {
    ...
    // ======================
    // 2. Convertir audio a base64 para Gemini
    // ======================
    ...
    // ======================
    // 3. Transcribir con Gemini API
    // ======================
    const GEMINI_API_KEY = Deno.env.get('GEMINI_API_KEY');
    if (!GEMINI_API_KEY) {
      throw new Error("Falta GEMINI_API_KEY en secretos");
    }

    console.log('🎤 Enviando audio a Gemini...');

    // Gemini API endpoint para audio
      ...
      contents: [
        {
          parts: [
            {
              text: "Transcribe el siguiente audio a texto. Solo devuelve el texto transcrito, sin explicaciones adicionales. El audio está en español."
            },
            {
              inline_data: {
    ...
    if (!geminiRes.ok) {
      ...
      throw new Error('Error al transcribir con Gemini: ' + errorText);
    }

    const geminiJson = await geminiRes.json();

    // Extraer transcripción de la respuesta de Gemini
    let transcript = '';
    if (geminiJson.candidates && geminiJson.candidates[0]?.content?.parts?.[0]?.text) {
      transcript = geminiJson.candidates[0].content.parts[0].text.trim();
    }

    if (!transcript) {
      console.error('❌ Respuesta de Gemini:', JSON.stringify(geminiJson));
      return new Response(
        JSON.stringify({
          success: false,
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: "Transacción creada desde voz usando Gemini",
        transcript,
        parsed,
        transaction,
      }),
